...
import { deleteById, getList } from 'api/voucher';
...
function EditAction({
    ...
    return (
        <Stack direction="row" sx={{ gap: 1, alignItems: 'center' }}>
            <Tooltip title={intl.formatMessage({ id: 'detail-voucher' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/voucher/detail/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Eye variant="Outline" />
                </IconButton>
            </Tooltip>

            <Tooltip title={intl.formatMessage({ id: 'edit-voucher' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/voucher/edit/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Edit2 variant="Outline" />
                </IconButton>
            </Tooltip>

            <Tooltip title={intl.formatMessage({ id: 'delete-voucher' })}>
...
export default function VoucherPage() {
    ...
    const table = useReactTable({
        ...
        getRowId: (row: Voucher) => (row.id ?? '').toString(),
        ...
        meta: {
            ...
            revertData: (rowIndex: number, revert: unknown) => {
                if (revert) {
                    setData((old: Voucher[]) => old.map((row, index) => (index === rowIndex ? originalData[rowIndex] : row)));
                } else {
        ...
        >
            ...
            >
                <Typography variant="h3" gutterBottom>
                    <FormattedMessage id="voucher-list" />
                </Typography>
                ...
                <Button
                    ...
                    startIcon={<Add />}
                >
                    <FormattedMessage id="add-voucher" />
                </Button>
            </Stack>
