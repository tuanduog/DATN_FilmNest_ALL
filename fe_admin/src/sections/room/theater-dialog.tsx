...
import { getList } from 'api/theater';
import MainCard from 'components/MainCard';
import { formatTime } from 'utils/formatDateTime';

interface TheaterDialogProps {
  open: boolean;
  id?: number;
  onClose: () => void;
  onConfirm: (theater: any) => void;
}
...
const TheaterDialog: React.FC<TheaterDialogProps> = ({ open, id, onClose, onConfirm }) => {
  ...
  const handleConfirm = () => {
    if (theater) {
      ...
      setAlert({
        open: true,
        message: intl.formatMessage({ id: 'please-select-one-theater' }),
        severity: 'warning'
      });
    }
  };
        ...
        >
          <MainCard content={false}>
            ...
            >
              ...
              >
                {alert?.message || intl.formatMessage({ id: 'no-notifications' })}
              </Alert>
            </Snackbar>
