...
function EditAction({
    ...
    return (
        <Stack direction="row" sx={{ gap: 1, alignItems: 'center' }}>
            <Tooltip title={intl.formatMessage({ id: 'detail-user' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/user/detail/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Eye variant="Outline" />
                </IconButton>
            </Tooltip>
...
export default function UserPage() {
    ...
    const columns = useMemo<ColumnDef<User>[]>(
        () => [
            ...
            {
                id: 'dob',
                header: intl.formatMessage({ id: 'dob' }),
                accessorKey: 'dob',
                dataType: 'text',
                enableGrouping: false,
                meta: { width: '35%' },
            ...
            {
                id: 'nationality',
                header: intl.formatMessage({ id: 'nationality' }),
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <Stack direction="row" alignItems="center" spacing={1}>
                            {country && <img loading="lazy" width="24" src={country.flag} alt="" style={{ border: '1px solid #eee' }} />}
                            <Typography>{country ? country.label : code}</Typography>
                        </Stack>
                    );
                }
            },
