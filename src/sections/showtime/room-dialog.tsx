...
import { getListByTheaterId } from 'api/room';
...
const RoomDialog: React.FC<RoomDialogProps> = ({ open, id, theaterId, onClose, onConfirm }) => {
    ...
    useEffect(() => {
        const fetchRooms = async () => {
            ...
            const response = await getListByTheaterId(theaterId, { ...pageRequest, status: 'ACTIVE' });
        ...
        fetchRooms();
    }, [pageRequest, intl, logout, theaterId]);
    ...
    const handleConfirm = () => {
        if (room) {
            ...
            setAlert({
                open: true,
                message: intl.formatMessage({ id: 'please-select-one-room' }),
                severity: 'warning'
            });
        }
    };
