...
const MovieDialog: React.FC<MovieDialogProps> = ({ open, id, onClose, onConfirm }) => {
            ...
            },
            {
                ...
                cell: (cell) => {
                    ...
                    return (
                        <Box
                            component="img"
                            src={typeof src === 'string' ? src : src instanceof File ? URL.createObjectURL(src) : '/assets/images/default-movie.png'}
                            alt="movie"
                            sx={{ width: 45, height: 65, borderRadius: 1, objectFit: 'cover', display: 'block' }}
                        />
                    );
                },
                enableGrouping: false,
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            const response = await getList({ ...pageRequest, status: 'ACTIVE' });
    ...
    const handleConfirm = () => {
        if (movie) {
            ...
            setAlert({
                open: true,
                message: intl.formatMessage({ id: 'please-select-one-movie' }),
                severity: 'warning'
            });
        }
    };
