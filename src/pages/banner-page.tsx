...
import { deleteById, getList } from 'api/banner';
...
function EditAction({
    ...
    return (
        <Stack direction="row" sx={{ gap: 1, alignItems: 'center' }}>
            <Tooltip title={intl.formatMessage({ id: 'detail-banner' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/banner/detail/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Eye variant="Outline" />
                </IconButton>
            </Tooltip>

            <Tooltip title={intl.formatMessage({ id: 'edit-banner' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/banner/edit/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Edit2 variant="Outline" />
                </IconButton>
            </Tooltip>

            <Tooltip title={intl.formatMessage({ id: 'delete-banner' })}>
...
export default function BannerPage() {
    ...
    const columns = useMemo<ColumnDef<Banner>[]>(
        () => [
            ...
            {
                id: 'name',
                header: intl.formatMessage({ id: 'banner-name' }),
                accessorKey: 'name',
                dataType: 'text',
                enableGrouping: false,
                meta: { width: '35%' }
    ...
    const table = useReactTable({
        ...
        getRowId: (row: Banner) => (row.id ?? '').toString(),
        ...
        meta: {
            ...
            revertData: (rowIndex: number, revert: unknown) => {
                if (revert) {
                    setData((old: Banner[]) => old.map((row, index) => (index === rowIndex ? originalData[rowIndex] : row)));
                } else {
        ...
        >
            ...
            >
                <Typography variant="h3" gutterBottom>
                    <FormattedMessage id="banner-list" />
                </Typography>
                ...
                <Button
                    ...
                    startIcon={<Add />}
                >
                    <FormattedMessage id="add-banner" />
                </Button>
            </Stack>
