...
import { deleteById, getList } from 'api/employee';
...
const fuzzyFilter: FilterFn<Employee> = (row, columnId, value, addMeta) => {
    // rank the item
...
function EditAction({
    ...
    const handleDelete = async () => {
        ...
        if (response.status == HttpStatusCode.Ok) {
            setAlert({ open: true, message: intl.formatMessage({ id: 'delete-employee-success' }), severity: 'success' });
            setReload(!reload);
    ...
    return (
        <Stack direction="row" sx={{ gap: 1, alignItems: 'center' }}>
            <Tooltip title={intl.formatMessage({ id: 'detail-employee' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/employee/detail/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Eye variant="Outline" />
                </IconButton>
            </Tooltip>

            <Tooltip title={intl.formatMessage({ id: 'edit-employee' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/employee/edit/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Edit2 variant="Outline" />
                </IconButton>
            </Tooltip>

            <Tooltip title={intl.formatMessage({ id: 'delete-employee' })}>
            ...
            >
                <DialogTitle id="alert-dialog-title">
                    {intl.formatMessage({ id: 'delete-employee-confirm' })}
                </DialogTitle>

                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        {intl.formatMessage({ id: 'delete-employee-description' })}
                    </DialogContentText>
                </DialogContent>
                ...
                >
                    <Stack direction="row" spacing={2} sx={{ flexGrow: 1, flexWrap: 'wrap' }}>
                        <OutlinedInput
                            ...
                            placeholder={intl.formatMessage({ id: 'search-employee-placeholder' })}
                            sx={{ minWidth: 100 }}
                            inputProps={{
                                sx: {
