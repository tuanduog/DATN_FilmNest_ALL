...
import { deleteById, getList } from 'api/membership';
...
function EditAction({
    ...
    return (
        <Stack direction="row" sx={{ gap: 1, alignItems: 'center' }}>
            <Tooltip title={intl.formatMessage({ id: 'detail-membership' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/membership/detail/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Eye variant="Outline" />
                </IconButton>
            </Tooltip>

            <Tooltip title={intl.formatMessage({ id: 'edit-membership' })}>
                <IconButton color="primary" onClick={() => navigate(`/admin/membership/edit/${row.id}`)} disabled={row.original.status == 'inactive'}>
                    <Edit2 variant="Outline" />
                </IconButton>
            </Tooltip>

            <Tooltip title={intl.formatMessage({ id: 'delete-membership' })}>
...
export default function MembershipPage() {
    ...
    const columns = useMemo<ColumnDef<Membership>[]>(
        () => [
            ...
            {
                id: 'name',
                header: intl.formatMessage({ id: 'membership-name' }),
                accessorKey: 'name',
                dataType: 'text',
                enableGrouping: false,
                meta: { width: '25%' }
    ...
    const table = useReactTable({
        ...
        getRowId: (row: Membership) => (row.id ?? '').toString(),
        ...
        meta: {
            ...
            revertData: (rowIndex: number, revert: unknown) => {
                if (revert) {
                    setData((old: Membership[]) => old.map((row, index) => (index === rowIndex ? originalData[rowIndex] : row)));
                } else {
                    setOriginalData((old) => old.map((row, index) => (index === rowIndex ? data[rowIndex] : row)));
                }
            },
            updateData: (rowIndex, columnId, value) => {
                setData((old: Membership[]) =>
                    old.map((row, index) => {
                        if (index === rowIndex) {
